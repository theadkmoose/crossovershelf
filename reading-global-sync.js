...
(() => {
  ...
  const normalizeStatus = value => ({ read:"read", reading:"currently-reading", "currently-reading":"currently-reading", want:"want-to-read", "want-to-read":"want-to-read", dnf:"dnf" }[value] || "unread");
  const legacyKey = (title, author = "") => `${clean(title).toLowerCase()}::${clean(author).replace(/^by\s+/i, "").toLowerCase()}`;
  ...
  function migrateLegacyRecords() {
    const history = load(HISTORY, {}), statusMap = load(STATUS_KEY, {});
    let changed = false;
    books().forEach(book => {
      if (!book?.id || !book.title) return;
      const canonical = String(book.id), oldKey = legacyKey(book.title, book.author), old = history[oldKey], current = history[canonical];
      if (old && !current) { history[canonical] = { ...old, bookId: canonical, title: book.title, author: book.author || "" }; delete history[oldKey]; changed = true; }
      else if (current && current.bookId !== canonical) { current.bookId = canonical; changed = true; }
  ...
  const ReadingStore = {
    getAll() { return Object.values(migrateLegacyRecords()).filter(record => record && record.title); },
    get(bookId) { return bookId ? (migrateLegacyRecords()[String(bookId)] || null) : null; },
    setStatus(bookId, status, metadata = {}) {
      if (!bookId) return null;
      const normalized = normalizeStatus(status), history = migrateLegacyRecords(), book = books().find(item => String(item?.id) === String(bookId)), key = String(bookId);
      const record = history[key] || { bookId:key, title:metadata.title || book?.title || "", author:metadata.author || book?.author || "" };
      ...
      history[key] = record; save(HISTORY, history);
      const statuses = load(STATUS_KEY, {}); statuses[bookId] = normalized; save(STATUS_KEY, statuses);
      window.dispatchEvent(new CustomEvent(SYNC_EVENT, { detail: record }));
      return record;
    },
    syncFromStatusMap(statusMap = {}) {
      const history = migrateLegacyRecords();
      let changed = false;
      books().forEach(book => {
        if (!book?.id || !book.title) return;
        const normalized = normalizeStatus(statusMap[book.id]);
        if (normalized === "unread") return;
        const key = String(book.id), record = history[key] || { bookId:key, title:book.title, author:book.author || "" };
        if (record.status !== normalized) { record.status = normalized; changed = true; }
        if (normalized === "currently-reading" && !record.startedAt) { record.startedAt = new Date().toISOString(); changed = true; }
        if (normalized === "read" && !record.finishedAt) { record.finishedAt = new Date().toISOString(); changed = true; }
        if (normalized === "dnf" && !record.dnfAt) { record.dnfAt = new Date().toISOString(); changed = true; }
        history[key] = record;
      });
      if (changed) save(HISTORY, history);
      window.dispatchEvent(new CustomEvent(SYNC_EVENT, { detail: { bulk:true } }));
      return history;
    },
    setRating(bookId, rating, metadata = {}) {
      if (!bookId) return null;
      const value = Math.max(1, Math.min(5, Number(rating) || 0)); if (!value) return null;
      const history = migrateLegacyRecords(), book = books().find(item => String(item?.id) === String(bookId)), key = String(bookId);
      const record = history[key] || { bookId:key, title:metadata.title || book?.title || "", author:metadata.author || book?.author || "" };
      record.rating = value; history[key] = record; save(HISTORY, history);
  ...
  function injectStyle() {
    ...
    const style = document.createElement("style"); style.id = "cs-rating-style";
    style.textContent = `.cs-rating-box{display:block!important;width:100%!important;box-sizing:border-box;margin:14px 0 18px;padding:16px 18px;border:1px solid var(--ink-line,#d9ceb0);border-radius:14px;background:rgba(200,154,60,.09);position:relative;z-index:5}.cs-rating-label{display:block;margin:0 0 8px;font:600 12px 'IBM Plex Mono',monospace;letter-spacing:.12em;text-transform:uppercase;color:inherit;opacity:.78}.cs-rating-row{display:flex!important;align-items:center!important;gap:3px!important;min-height:48px}.cs-rating-star{display:inline-flex!important;align-items:center!important;justify-content:center!important;width:44px!important;height:44px!important;padding:0!important;margin:0!important;border:0!important;background:transparent!important;box-shadow:none!important;border-radius:8px!important;font-size:34px!important;line-height:1!important;cursor:pointer!important;touch-action:manipulation!important;color:#9a9a9a!important;opacity:1!important}.cs-rating-star.is-on{color:#c89a3c!important}.cs-rating-value{margin-left:9px;font:500 13px 'IBM Plex Mono',monospace;opacity:.8}.cs-rating-star:active{transform:scale(.9)}.cs-rating-star:focus-visible{outline:2px solid #c89a3c;outline-offset:2px}@media(max-width:700px){.cs-rating-box{padding:14px 15px}.cs-rating-star{width:42px!important;height:42px!important;font-size:32px!important}}`;
  ...
  function readButton(button) { return button && visible(button) && clean(button.textContent).toLowerCase()==="read"; }
