...
(() => {
  ...
  const daysAgo = d => d ? Math.max(0, Math.floor((Date.now()-new Date(d).getTime())/86400000)) : 99999;
  ...
  function setStatus(book,status){const d=load(),k=keyFor(book.title,book.author),old=d[k]||{title:book.title,author:book.author,createdAt:new Date().toISOString()};old.status=status;if(status==="currently-reading"&&!old.startedAt)old.startedAt=new Date().toISOString();if(status==="read")old.finishedAt=new Date().toISOString();if(status==="dnf")old.dnfAt=new Date().toISOString();d[k]=old;save(d);renderModalControls();renderTab("history")}
  function setRating(book,rating){const d=load(),k=keyFor(book.title,book.author),old=d[k]||{title:book.title,author:book.author,status:book.read?"read":"want-to-read",createdAt:new Date().toISOString()};old.rating=rating;d[k]=old;save(d);renderModalControls()}

  function renderModalControls(){
    const book=currentBook(); if(!book)return;
    const modal=book.modal; let box=modal.querySelector(".cs-modal-reading"); if(!box){box=document.createElement("section");box.className="cs-modal-reading";const body=modal.querySelector(".modal-body");if(body)body.appendChild(box);else modal.appendChild(box)}
    const rec=statusFor(book); const status=rec?.status||"want-to-read";
    box.innerHTML=`<div class="cs-modal-reading-title">My Reading Status</div><div class="cs-status-grid">${[["want-to-read","Want to Read"],["currently-reading","Currently Reading"],["read","Read"],["dnf","Did Not Finish"]].map(([v,l])=>`<button class="cs-status-btn ${status===v?'active':''}" data-status="${v}">${l}</button>`).join("")}</div><div class="cs-rating" style="margin-top:9px"><span style="font-size:.7rem;color:var(--text-dim,#a7acb7);margin-right:7px">My rating</span>${[1,2,3,4,5].map(n=>`<button data-rating="${n}" class="${(rec?.rating||0)>=n?'on':''}" aria-label="${n} stars">★</button>`).join("")}</div>`;
    box.querySelectorAll("[data-status]").forEach(b=>b.addEventListener("click",()=>setStatus(book,b.dataset.status)));
    box.querySelectorAll("[data-rating]").forEach(b=>b.addEventListener("click",()=>setRating(book,Number(b.dataset.rating))));
  }

  function records(){return Object.values(load()).filter(x=>x&&x.title)}
  function renderHistory(){
    const all=records().sort((a,b)=>new Date(b.finishedAt||b.startedAt||b.createdAt||0)-new Date(a.finishedAt||a.startedAt||a.createdAt||0));
  ...
  async function globalAI(){
    ...
    const r=records(),read=r.filter(x=>x.status==="read"),tbr=r.filter(x=>x.status==="want-to-read"),seed=read.slice().sort((a,b)=>(b.rating||0)-(a.rating||0))[0]||read[0];
    if(!seed){alert("Mark at least one book Read so AI can personalize your recommendations.");return}
  ...
  function watchModal(){
    const observer=new MutationObserver(()=>{const b=currentBook();if(b){const k=keyFor(b.title,b.author);if(k!==lastBookKey){lastBookKey=k;setTimeout(renderModalControls,30)}else if(!document.querySelector(".cs-modal-reading"))renderModalControls();}});
