...
const CACHE_VERSION = "v23-scoring-menu";
const SHELL_CACHE = `crossover-shelf-shell-${CACHE_VERSION}`;
const RUNTIME_CACHE = `crossover-shelf-runtime-${CACHE_VERSION}`;
const MY_READING = "./my-reading.js?v=23";
const AI = "./ai-recommendations.js?v=23";
const SHELL_CONTROLS = "./shell-controls.js?v=23";
...
async function injectModules(response){
 if(!response||!response.ok)return response;const type=response.headers.get("content-type")||"";if(!type.includes("text/html"))return response;const html=await response.clone().text();const marker="<!-- crossover-shelf-enhancements-v23 -->";if(html.includes(marker))return response;
