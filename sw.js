...
const CACHE_VERSION = "v12-my-reading-detail";
...
self.addEventListener("activate",event=>event.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k!==SHELL_CACHE&&k!==RUNTIME_CACHE).map(k=>caches.delete(k)))).then(()=>self.clients.claim()).then(()=>self.clients.matchAll({type:"window",includeUncontrolled:true})).then(clients=>Promise.all(clients.map(client=>client.navigate?client.navigate(client.url).catch(()=>undefined):undefined)))));
...
async function injectModules(response){
 ...
 if(html.includes(RATING_SCRIPT)&&html.includes(GLOBAL_READING_SCRIPT)&&html.includes(READING_SCRIPT))return response;
 const injected=html.replace(/<\/body>/i,`<script>try{if(!localStorage.getItem("crossoverShelfCoverCacheResetV1")){localStorage.removeItem("crossover-shelf-cover-cache-v1");localStorage.setItem("crossoverShelfCoverCacheResetV1","1")}}catch(e){}</script><script>try{window.CrossoverShelfBooks=BOOKS;window.CrossoverShelfReadingStatus=readingStatus;}catch(e){}</script><style id="cs-ai-layout-fix">#cs-ai-fab{bottom:calc(78px + env(safe-area-inset-bottom)) !important;}</style><script src="${AI_SCRIPT}"></script><script src="${GLOBAL_READING_SCRIPT}"></script><script src="${READING_SCRIPT}"></script><script src="${RATING_SCRIPT}"></script></body>`);
 if(injected===html)return response;const headers=new Headers(response.headers);headers.set("content-type","text/html; charset=utf-8");headers.delete("content-length");return new Response(injected,{status:response.status,statusText:response.statusText,headers});
}
self.addEventListener("fetch",event=>{const req=event.request;if(req.method!=="GET")return;const url=new URL(req.url);
 if(url.origin===self.location.origin){const doc=url.pathname.endsWith("/index.html")||url.pathname.endsWith("/");event.respondWith(caches.match(req).then(async cached=>{if(cached)return doc?injectModules(cached):cached;try{const r=await fetch(req),copy=r.clone(),c=await caches.open(SHELL_CACHE);await c.put(req,copy);return doc?injectModules(r):r}catch(_){const f=await caches.match("./index.html");return doc?injectModules(f):f;}}));return;}
