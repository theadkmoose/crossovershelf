...
const CACHE_VERSION = "v39-king-insights-deterministic";
const SHELL_CACHE = `crossover-shelf-shell-${CACHE_VERSION}`;
const RUNTIME_CACHE = `crossover-shelf-runtime-${CACHE_VERSION}`;
const MY_READING = "./my-reading.js?v=24";
const AI = "./ai-recommendations.js?v=24";
const SHELL_CONTROLS = "./shell-controls-v27.js?v=39";
const KING_INSIGHTS = "./king-insights-v2.js?v=38";
const STATS_INIT = "./stats-enhancement-init.js?v=24";
const RANK_MOVEMENT = "./rank-movement.js?v=26";
const RF_MODAL = "./rfantasy-modal-fix-v8.js?v=34";
const CORE_ASSETS = ["./","./index.html","./my-reading.js","./ai-recommendations.js","./shell-controls-v27.js","./king-insights-v2.js","./stats-enhancement-init.js","./rank-movement.js","./rfantasy-modal-fix-v8.js","./rfantasy-2025-10plus.csv","./manifest.webmanifest","./icon-180.png","./icon-192.png","./icon-512.png"];
...
async function injectModules(response){if(!response||!response.ok)return response;const type=response.headers.get("content-type")||"";if(!type.includes("text/html"))return response;const html=await response.clone().text();const marker="<!-- crossover-shelf-enhancements-v39-king-insights-deterministic -->";if(html.includes(marker))return response;const injected=html.replace(/<\/body>/i,`${marker}<script>try{window.CrossoverShelfBooks=BOOKS}catch(e){}</script><script src="${AI}" defer></script><script src="${MY_READING}" defer></script><script src="${SHELL_CONTROLS}" defer></script><script src="${KING_INSIGHTS}" defer></script><script src="${STATS_INIT}" defer></script><script src="${RANK_MOVEMENT}" defer></script><script src="${RF_MODAL}" defer></script></body>`);const headers=new Headers(response.headers);headers.set("content-type","text/html; charset=utf-8");headers.delete("content-length");return new Response(injected,{status:response.status,statusText:response.statusText,headers})}
