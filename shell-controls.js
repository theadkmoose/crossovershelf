...
(() => {
  ...
  const boot = () => {
    ...
    const style = document.createElement("style"); style.id="cs-shell-style";
    style.textContent=`
      ...
      #cs-my-reading-btn{font:inherit;border:1px solid var(--ink-line,#363b47);background:transparent;color:inherit;border-radius:999px;padding:7px 11px;cursor:pointer;margin-left:7px}
      #cs-my-reading-btn:hover{border-color:var(--brass,#c89a3c)}
    ...
    const nav=document.querySelector("nav"); if(nav){
      if(!document.getElementById("cs-refresh-btn")){const b=document.createElement("button");b.id="cs-refresh-btn";b.type="button";b.textContent="↻ Refresh";b.title="Refresh Crossover Shelf";b.onclick=()=>location.reload();nav.appendChild(b);}
      if(!document.getElementById("cs-my-reading-btn")){const b=document.createElement("button");b.id="cs-my-reading-btn";b.type="button";b.textContent="▦ My Reading";b.title="My Reading";b.onclick=()=>window.CrossoverShelfMyReading?.open?.();nav.appendChild(b);}
    }
  };
